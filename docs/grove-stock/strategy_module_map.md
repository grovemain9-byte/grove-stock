...
| 2-C | セクター連れ高(出遅れ順張り) | ❌RED 2026-06-14 | S1ゲート: コストON でedge消滅(ann_Sharpe 0.45→0.10、CIゼロ跨ぎ)、逆張りbaseline(0.70)に全指標で劣る。look-ahead排除済。詳細 `research/parallel-s1-gate-batch1.md` | KILL |
| 2-D | 撤退速度(反発待たず即損切り) | ❌RED 2026-06-14 | S1ゲート: 速く/厳しく切る改善なし(rebound-fail撤退は悪化 Sharpe0.60<0.77)、逆にlooser stop-7%が最良=仮説と逆。詳細 `research/parallel-s1-gate-batch1.md` | KILL(現stop-5%維持) |
...
| 2-H | 目標値なし原則(trailing exit) | 🟡YELLOW 2026-06-14 | S1ゲート: trailingはDD減が構造的に本物(2024-08 -19%→-14〜16%)だが利確edgeはノイズ(CI重複)、勝率57%→43-50%低下。**守り(DD防御)として価値、攻め(利確上積み)はnull**。詳細 `research/parallel-s1-gate-batch1.md` | DD防御目的なら設計検討。要portfolio concurrency正しいsim |
| 2-I | regime-conditional 逆張りsizing(弱気厚張り) | ⏸️defer 2026-06-14 | S1 tail-gate **RED**(独立評価opus): 見かけ「×2が勝つ(calmar 5.42>4.54)」は **レバレッジ交絡(現金ゲート無=最大200%展開)+非年率化(+0.88→年率+0.04≒ノイズ)+反発暴落1回依存(6日V字)** の合成。元の破滅仮説は非反発暴落で未検証。詳細 `docs/grove-stock/research/2-I-tail-gate.md` | 再開=①現金/証拠金ゲートor等展開正規化 ②非反発暴落stress ③年率化edgeがノイズ超 を全て満たせば(capは左尾防壁として保持)設計へ |
...
| 3-3 | 新高値ブレイクアウト(順張り) | 🟡YELLOW 2026-06-14 | モメンタム自己強化(CANSLIM)。S1: 実alpha~+16%(β0.45)はあるがSharpe水増し(真MTM1.47<報1.73)・2.0x基準未達・survivor bias・単一強気未検証→**portfolio再sim要** | 月3-10%(非統計)→実測CAGR+27%だが半分はβ | 近(価格のみ)。再sim: 真MTM curve+時点別universe+regime分割 |
| 3-4 | 優待・配当先回り | 🔵研究済 | 権利日40営業日前の季節上昇 | 権利月+5-15% | 中(配当カレンダー) |
| 3-5 | ペアトレード(統計裁定) | ❌RED 2026-06-14 | S1: 真MTM Sharpe -0.02、avg/trade CIゼロ跨ぎ、cost-ONで-5.8%/年。β-0.009で中立は確認も中立≠edge。2021-23選定ペアがBOJ利上げで崩壊。詳細 `research/parallel-s1-gate-batch2.md` | KILL(in-sample coint≠OOS reversion) |
| 3-6 | 空売り(逆張り対称化) | ❌RED 2026-06-14 | S1: 全regimeで損(CAGR-93%/WR43%)。逆張りedgeは非対称=買われ過ぎ株は強気相場で上昇継続。詳細 `research/parallel-s1-gate-batch2.md` | KILL(long edgeの符号反転は成立せず) |

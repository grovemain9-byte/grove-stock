...
# → 探索クエリ (Q7-Q11) 用。era全体のトレンドを見る。
SINCE="DATE '2026-05-17'"
# kill_criterion 登録の観察窓 (2026-05-25 〜): c9254b4 修正後のみ。GO/KILL 判定 (Q1-Q6 +
# 簡易判定) はこちらを使う。SINCE(5/17) を使うと 5/20 の修正前データ (consensus=5 が
# 60件全pass) が混入し K1/K5 を誤発火させる (2026-05-30 中間レビューで検出)。
KILL_SINCE="DATE '2026-05-25'"
PY=".venv/bin/python"

echo "=========================================="
echo "  forward paper verification (令和式 era 起点 2026-05-17, kabu commission ¥0)"
echo "  探索クエリ(Q7-Q11) since: $SINCE"
echo "  GO/KILL判定(Q1-Q6)  since: $KILL_SINCE  (kill_criterion登録窓)"
echo "=========================================="
echo
...
FROM positions
WHERE book=\\'p50m\\' AND entry_date >= $KILL_SINCE
...
FROM positions
WHERE status=\\'closed\\' AND entry_date >= $KILL_SINCE
''')
...
FROM decision_shadow
WHERE decision=\\'pass\\' AND decided_date >= $KILL_SINCE
...
FROM positions
WHERE book=\\'p50m\\' AND status=\\'closed\\' AND entry_date >= $KILL_SINCE
''')
...
FROM decision_shadow WHERE consensus=5 AND decided_date >= $KILL_SINCE
''').fetchone()[0]
...
FROM decision_shadow WHERE decision=\\'pass\\' AND decided_date >= $KILL_SINCE
''').fetchone()[0]
